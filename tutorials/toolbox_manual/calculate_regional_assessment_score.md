...
Adding and registering a new data layer as described previously, and subsequently running the Toolbox App will allow R to read the layer but will not incorporate the new data into the goal calculations. It will thus now be necessary to update the models and goals for which the new data layers were created.
